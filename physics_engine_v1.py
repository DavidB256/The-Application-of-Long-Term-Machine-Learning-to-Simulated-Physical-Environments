...
class Rect(Solid):
    def __init__(self, pos_derivs=None, rot_derivs=None, height=10, width=10, mass=10, static=False):
...
solids = []
# solids.append(Rect())
solids.append(Rect(pos_derivs=[[250, 250], [10, -30], [0, 9.81], [0, 0]], height=5, width=5))
# solids.append(Rect(pos_derivs=[[499, 30], [0, 0], [0, 0], [0, 0]], height=50, width=50))
solids.append(Ellipse(pos_derivs=[[100, 100], [5, 0], [0, 0], [0, 0]]))

# run simulation to output into output.txt, number of lines of output.txt equals len(solids)+len(solids)(seconds/tick_len)
for i in range(int(seconds / tick_len)):
    for j in range(len(solids)):
        # creates txt output
        f.write(str(j) + '?' +
                'pos_derivs' + str(solids[j].pos_derivs) + '?\n')
        solids[j].update(tick_len)
